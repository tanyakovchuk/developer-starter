...
(() => {
  ...
  window.Webflow.push(() => {
    ...
    if (!getCookie("extra_metadata")) {
      const buttons = document.querySelectorAll("a");
      buttons.forEach(function(button) {
        const href = button.getAttribute("href");
        if (href && href.startsWith("https://app.paperwriters24.com/auth/register")) {
          button.addEventListener("click", function(event) {
            event.preventDefault();
            handleAuthPopupClick(1);
          });
          button.classList.add("js-sign-in-popup");
        } else if (href && href.startsWith("https://app.paperwriters24.com/auth/login")) {
          button.addEventListener("click", function(event) {
            event.preventDefault();
            handleAuthPopupClick(0);
          });
          button.classList.add("js-log-in-popup");
        }
      });
    }
    ...
    AppleID.auth.init({
      ...
      scope: "name email",
      redirectURI: "https://paperwriters24.com/apple-callback",
      state: "init",
    ...
    function signInWithApple() {
      ...
      userMethodRegisterEvent("click_apple_icon");
    }
    ...
    document.querySelectorAll(".auth-form__tabs__link.w-tab-link").forEach((tabLink) => {
    ...
    function sendFbRequest(activateValue) {
      fetch("https://app.paperwriters24.com/api/social/auth/signin/facebook/", {
        method: "POST",
        headers: {
        ...
        if (data.is_sms_otp_allow) {
          ...
          if (data.is_new) {
            setCookie({ name: "new_user", value: "true", domain: ".paperwriters24.com" });
          }
          fetchUserData().then((userData) => {
            ...
            if (!userData.allow_unverified_user && userData.state === "unverified") {
              showConfirmEmailStep(userData.email);
            } else {
              window.location.href = "https://app.paperwriters24.com/";
            }
          });
        } else {
    ...
    function sendAppleRequest(token, activateValue) {
      fetch("https://app.paperwriters24.com/api/social/auth/signin/apple-id/", {
        method: "POST",
        body: JSON.stringify({
          access_token: token,
          client_id: "pgnfidgwuKhlfrh4jRuIirRpvteChDGnga9I4hmj",
    ...
    function fetchUserData() {
      ...
      return fetch("https://app.paperwriters24.com/api/auth/user/", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    function switchTab(tabIndex) {
      document.querySelectorAll(".auth-form__tabs__link.w-tab-link")[tabIndex].click();
      setTimeout(() => {
    ...
    function handleApiResponse(data, emailInputWrap, passwordInputWrap, passErrorText, emailErrorText) {
      if (data.is_sms_otp_allow) {
        showTwoFactorStep();
        loginToken = data.token;
      } else if (!data.error_map) {
    ...
    function validateForm(email, password, emailInputWrap, emailErrorText, passwordInputWrap, passErrorText) {
      let isValid = true;
    ...
    document.querySelectorAll(".js-toggle-password").forEach((icon) => {
      icon.addEventListener("click", function() {
        ...
        const passwordHideIcon = parent.querySelector(".auth-form__input-hide-pass");
        const passwordShowIcon = parent.querySelector(".auth-form__input-show-pass");
    ...
    document.getElementById("reactivate-no").addEventListener("click", () => toggleIsActive("reactivate-popup", false));
    document.getElementById("contact-help-btn").addEventListener("click", () => {
      Intercom("show");
      toggleIsActive("locked-popup", false);
    });
    document.getElementById("reactivate-yes").addEventListener("click", function() {
      ...
      fetch("https://app.paperwriters24.com/api/auth/signin/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        ...
        setCookie({ name: "accessToken", value: data.access_token, domain: ".paperwriters24.com" });
        setCookie({
          name: "refreshToken",
          value: data.refresh_token,
          domain: ".paperwriters24.com"
        });
        handleApiResponse(data, emailInputWrap, passwordInputWrap, passErrorText, emailErrorText);
      }).catch((error) => console.error("Error:", error));
      toggleIsActive("reactivate-popup", false);
    });
    document.getElementById("reactivate-yes-google").addEventListener("click", function() {
      const accessToken = getCookie("accessToken");
      const authForm = document.getElementById("auth-form");
      const isPremiumUser = authForm && authForm.classList.contains("is-active");
      fetch("https://app.paperwriters24.com/api/social/auth/signin/google-oauth2/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
    ...
    document.getElementById("reactivate-yes-fb").addEventListener("click", function() {
      sendFbRequest(true);
      toggleIsActive("reactivate-popup", false);
    });
    document.getElementById("reactivate-yes-apple").addEventListener("click", function() {
    ...
    function validateLoginEmail() {
      const { email, emailInputWrap, emailErrorText } = getLoginEmailValidationElements();
      validateForm(email, "", emailInputWrap, emailErrorText, null, null);
    }
    function validateLoginPassword() {
      const { password, passwordInputWrap, passErrorText } = getLoginPasswordValidationElements();
    ...
    document.getElementById("loginButton").addEventListener("click", function(event) {
      ...
      const { email, emailInputWrap, emailErrorText } = getLoginEmailValidationElements();
      const { password, passwordInputWrap, passErrorText } = getLoginPasswordValidationElements();
      const isEmailValid = validateForm(email, "", emailInputWrap, emailErrorText, null, null);
    ...
    document.getElementById("signin-email").addEventListener("input", validateLoginEmail);
    document.getElementById("signin-password").addEventListener("input", validateLoginPassword);
    ...
    function validateSignEmail() {
      const { email, emailInputWrap, emailErrorText } = getSignEmailValidationElements();
      validateForm(email, "", emailInputWrap, emailErrorText, null, null);
    }
    ...
    document.getElementById("signUpButton").addEventListener("click", function(event) {
      ...
      if (!isEmailValid || !isPasswordValid)
        return;
      toggleLoader(currentBtn, true);
      fetch("https://app.paperwriters24.com/api/auth/signup/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
    ...
    document.getElementById("signup-email").addEventListener("input", validateSignEmail);
    document.getElementById("signup-password").addEventListener("input", validateSignPassword);
    ...
    function validateForgotEmail() {
      const { email, emailInputWrap, emailErrorText } = getForgotEmailValidationElements();
      validateForm(email, "", emailInputWrap, emailErrorText, null, null);
    }
    document.getElementById("forgotButton").addEventListener("click", function() {
      ...
      if (!isEmailValid)
        return;
      fetch(
        `https://app.paperwriters24.com/api/auth/forgot-password?email=${encodeURIComponent(email)}`,
        {
          method: "GET"
        }
    ...
    document.getElementById("forgot-email").addEventListener("input", validateForgotEmail);
    ...
    document.getElementById("backToSignup").addEventListener("click", function() {
      ...
      toggleIsActive("initial-state", true);
      toggleIsActive("auth-form-social", true);
      fetch("https://app.paperwriters24.com/api/auth/signout/", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${accessToken}`
    ...
    document.getElementById("confirmEmailBtn").addEventListener("click", function() {
      ...
      const accessToken = getCookie("accessToken");
      fetch("https://app.paperwriters24.com/api/auth/verify-email/resend/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${accessToken}`
        ...
        if (data.ok) {
          document.getElementById("authFormConfirmBtn").classList.add("is-hide");
          toggleIsActive("authFormConfirmDone", true);
        }
      }).catch((error) => console.error("Error:", error));
    });
    document.getElementById("confirmEmailAgain").addEventListener("click", function() {
    ...
    codeInputs.forEach((input, index) => {
      ...
      input.addEventListener("paste", (e) => {
        ...
        if (pasteData.length === codeInputs.length) {
          codeInputs.forEach((input2, i) => {
            input2.value = pasteData[i] || "";
          });
          checkInputs();
    ...
    document.getElementById("twoFactorCodeButton").addEventListener("click", function(event) {
      const currentBtn = event.currentTarget;
      const codeInputWrap = document.getElementById("code-input-wrap");
      const codeInputError = document.getElementById("code-input-error");
      if (!code) {
        handleFormError(codeInputWrap, codeInputError, "Verification code is required");
        return;
      }
      toggleLoader(currentBtn, true);
      fetch("https://app.paperwriters24.com/api/auth/signin/two-factor/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        ...
        if (!data.error_map) {
          ...
          clearCodeInputs();
          handleFormError(codeInputWrap, codeInputError, data.message?.otp?.[0] || "");
        }
      }).catch((error) => console.error("Error:", error)).finally(() => {
        toggleLoader(currentBtn, false);
      });
    ...
    document.getElementById("closeCountryAll").addEventListener("click", function() {
      toggleIsActive("authFormCountry", false);
      toggleIsActive("auth-form", false);
    });
    ...
    function pushEvent(event, buttonText, clickText, type) {
      window.dataLayer.push({
        event,
        button_text: buttonText || void 0,
        click_text: clickText,
        type: type || void 0,
        subject: typeof selectedSubject !== "undefined" ? selectedSubject : null
      });
    }
    const buttonsConfig = [
      // Tabs
      {
        selector: ".auth-form__tabs__link.w-tab-link",
        index: 1,
      ...
      // Nav login/signup
      {
        selector: ".s-nav__login",
        event: "click_cta_on_landing_page",
        clickText: "Log in",
        type: "Login block"
      },
      {
        selector: ".s-nav__signup",
        event: "click_cta_on_landing_page",
        clickText: "Sign up",
        type: "Login block"
      },
      // Landing buttons
      {
        selector: '[data-button="get_help_hero"]',
        event: "landing_page_click_start_order",
        buttonText: "Get help",
        clickText: "Hero block"
      },
      {
        selector: '[data-button="get_started"]',
        event: "landing_page_click_start_order",
        buttonText: "Get started",
        clickText: "Writers block"
      },
      {
        selector: '[data-button="get_help_reviews"]',
        event: "landing_page_click_start_order",
        buttonText: "Get help",
        clickText: "Reviews block"
      },
      {
        selector: '[data-button="place_order"]',
        event: "landing_page_click_start_order",
        buttonText: "Place order",
        clickText: "How it works block"
      },
      {
        selector: '[data-button="get_help_why_us"]',
        event: "landing_page_click_start_order",
        buttonText: "Get help",
        clickText: "Why us block"
      },
      {
        selector: '[data-button="get_help_advantages"]',
        event: "landing_page_click_start_order",
        buttonText: "Get help",
        clickText: "Advantages block"
      },
      {
        selector: '[data-button="write_my_paper_footer"]',
        event: "landing_page_click_start_order",
        buttonText: "Write my paper",
        clickText: "Footer block"
      }
    ];
    buttonsConfig.forEach(({ selector, index, event, buttonText, clickText, type }) => {
      ...
      if (typeof index === "number" && elements[index]) {
        elements[index].addEventListener(
          "click",
          () => pushEvent(event, buttonText, clickText, type)
        );
      } else {
        elements.forEach(
          (el) => el.addEventListener("click", () => pushEvent(event, buttonText, clickText, type))
        );
      }
    });
